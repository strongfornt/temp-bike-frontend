...
import { Button, Card, Form, Input, InputNumber, Modal, Select, Tag, Upload } from "antd";
import TextArea from "antd/es/input/TextArea";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import BTable from "../../../components/BTable";
import { useAddProductMutation, useDeleteProductMutation, useGetProductsQuery, useUpdateProductMutation } from "../../../redux/features/product/productApi";
...
const ProductManagement = () => {
    ...
    const columns = [
        ...
        { title: "In Stock", dataIndex: "inStock", key: "inStock", render: (inStock: boolean) => (inStock ? "Yes" : "No") },
        {
            title: "Action",
            key: "action",
            render: (record: any) => (
        ...
        <>
            <div className="px-4 py-2">
                <div className="flex justify-between items-center my-4">
                    ...
                    <Button icon={<PlusOutlined />} size="middle" type="primary" onClick={() => { setVisible(true); setIsEdit(false); setImageList([]); }}>
                        Add Product
                    </Button>
                </div>
                ...
                <Card className="w-full">
                    <Form form={form} onFinish={onFinish} layout="vertical">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                            <Form.Item label="Product Name" name="name" rules={[{ required: true, message: "Please enter the product name" }]} className="col-span-2">
                                <Input placeholder="Enter product name" />
                            </Form.Item>

                            <Form.Item label="Brand" name="brand" rules={[{ required: true, message: "Please enter the brand" }]}>
                                <Select placeholder="Select a brand" options={[
                            ...
                            <Form.Item label="Category" name="category" rules={[{ required: true, message: "Please enter the model" }]}>
                            ...
                            <Form.Item label="Quantity" name="quantity" rules={[{ required: true, message: "Please enter the stock quantity" }]}>
                            ...
                            {!isEdit && (
                                <Form.Item label="Product Image"
                                    ...
                                    <Upload listType="picture-card" accept="image/*" fileList={imageList} onChange={handleImageChange} beforeUpload={() => false} maxCount={1}>
